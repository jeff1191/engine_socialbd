...
object DataTypeFactory {
  ...
  def getJsonString(enrichmentObj: RawObj, ins:String): String ={
    ins match {
      case Instructions.GET_JSON_AIR => compactRender((Extraction decompose  enrichmentObj.asInstanceOf[Air]))
      case Instructions.GET_JSON_TRAFFIC => compactRender((Extraction decompose   enrichmentObj.asInstanceOf[InterUrbanTraffic]))
      case Instructions.GET_JSON_TWITTER => compactRender((Extraction decompose   enrichmentObj.asInstanceOf[UrbanTraffic]))
      case Instructions.GET_JSON_BICIMAD => compactRender((Extraction decompose   enrichmentObj.asInstanceOf[BiciMAD]))
      case Instructions.GET_JSON_EMTBUS => compactRender((Extraction decompose enrichmentObj.asInstanceOf[EMTBus]))
      case _ => throw new Exception(s"Cannot create json object since ${ins} doesn't match")
    }
  }
}
