...
class AirETL(socialBDProperties: SocialBDProperties) extends ETL{
  ...
  override def process(): Unit = {
    ...
    val streamTwitterKafka: DataStream[String]= env.addSource(
...
//    transports.add(new InetSocketAddress(InetAddress.getLocalHost(),9300))
    ...
    streamTwitterKafka.addSink(new ElasticsearchSink(config, transports, new ElasticsearchSinkFunction[String] {
      def createIndexRequest(element: String): IndexRequest = {
        Requests.indexRequest.index(socialBDProperties.qualityAirConf.elasticIndex).
          `type`(socialBDProperties.qualityAirConf.elasticType).source(element)
      }
