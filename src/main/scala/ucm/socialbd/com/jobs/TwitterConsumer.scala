...
object TwitterConsumer {
  ...
  def main(args: Array[String]) {
    ...
    val streamTwitterKafka: DataStream[String]= env.addSource(new FlinkKafkaConsumer010[String]("tweets", new SimpleStringSchema(), properties))
    ...
    streamTwitterKafka.addSink(new ElasticsearchSink(config, transports, new ElasticsearchSinkFunction[String] {
      def createIndexRequest(element: String): IndexRequest = {
        ...
        Requests.indexRequest.index("tweets-spain-idx").`type`("tweets").source(element)
      }
    ...
    env.execute("Twitter consumer SocialBigData")
  }
